...
\begin{itemize}[label=\ding{43}]
 ...
 \item « \texttt{prends( )} » (ou appui sur touche P) : le robot ramasse une bille sur la case où il se trouve.
  \item « \texttt{mur\_devant( )} » renvoie un booléen indiquant si un mur se trouve devant le robot.
   \item « \texttt{bille\_au\_sol( )} » renvoie un booléen indiquant si une  bille se trouve sur la même case que le robot.
\end{itemize}
...
\begin{center}
 \includegraphics[scale=0.5]{images/scene3.png}
\end{center}


\item  Ouvrir la scène \texttt{scene0.wld} et commenter le dernier appel de fonction.


\begin{enumerate}
\item Écrire une fonction \texttt{demi\_tour()} qui fait faire un demi-tour sur lui-même au robot.

\item Écrire une fonction \texttt{bille\_devant()} qui fait avancer le robot sur la case devant (en supposant qu'un test précédent a permis de déterminer qu'elle existe), teste la présence d'une bille au sol avec \texttt{bille\_au\_sol()} puis le ramène à sa position initiale (avec la même orientation).

\item Écrire une fonction \texttt{petit\_poucet()} qui permet au robot de déposer une bille sur chacune des cases d'une scène carrée de dimensions $n \times n$, où $n$ est un entier impair. Le robot doit parcourir la scène en spirale depuis le coin inférieur gauche (Voir  figure 2 ci-dessous).

On commencera par placer $n^2$ billes ($n=5$ pour \texttt{scene0.wld})  dans la poche du  robot depuis le menu \texttt{Scène/Nombre de billes}.
\end{enumerate}
...
\begin{center}
\includegraphics[scale=0.8]{images/petit_poucet.png}
\end{center}
