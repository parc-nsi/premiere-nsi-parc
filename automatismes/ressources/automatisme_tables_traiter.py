...
les fonctions :
    ...
    Pour les tris il faut aussi compléter le programme principal en lignes 162 et 165
"""
...
def moyenne_table(table_eleves) :
    ...
    for enreg in table_eleves:
        somme  = somme + float(enreg['moyenne'])
        compteur = compteur + 1
...
def generer_table_recompenses(table_eleves):
    ...
    """
    table_recompenses = [enreg_avec_recompense(enreg) for enreg in table_eleves]   
...
def decompte_recompenses(table_recompenses):
    ...
    for enreg in table_recompenses:
        if enreg['récompense'] not in decompte:
            decompte[enreg['récompense']]  = 1
        else:
            decompte[enreg['récompense']] += 1
    return decompte
...
def clef_tri_moyenne_decroissant(enreg):
    ...
    return -int(enreg['moyenne'])

def clef_tri_alphabetique_croissant_moyenne_decroissant(enreg):
    """Clef de tri par moyenne décroissante
    Attention la valeur de l'attribut 'moyenne' de enreg est de type str"""
    return (enreg['nom'], enreg['prénom'] , -int(enreg['moyenne']))
...
table_tri1 = sorted(table_recompenses, key = clef_tri_moyenne_decroissant)
...
table_tri2 = sorted(table_recompenses, key = clef_tri_alphabetique_croissant_moyenne_decroissant)
