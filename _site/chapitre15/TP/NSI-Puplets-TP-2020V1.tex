...
\fancyhead[C]{\textbf{TP : p-uplets et fichiers textes}}
...
{\huge TP : p-uplets et fichiers textes}
...
\section{Traitement d'un fichier texte}

Un fichier texte contient une séquence de caractères, chaque caractère étant codé par une série d'octets selon un certain encodage. Nous allons manipuler dans ce TP trois fichiers  textes fournis dans l'archive \texttt{materiel.zip} qu'il faut donc déballer.

\begin{itemize}
	\item \texttt{communes69.csv}  contient  les informations du recensement 2015 pour les 296 communes du Rhône. Chaque ligne correspond à une commune avec les  informations \textit{nom,population municipale, population à part, population totale} séparées par une virgule.  L'extension \href{https://fr.wikipedia.org/wiki/Comma-separated_values}{csv} signifie \textit{Comma Separated Values} et désigne les fichiers textes ainsi formatés.
	
Voici les trois premières lignes :
	
\begin{lstlisting}[style=compil,numbers=left]
Affoux,340,3,343
Aigueperse,246,1,247
Albigny-sur-Saône,2793,37,2830
\end{lstlisting}

   \item \texttt{airports-reduit.csv} contient des informations sur les 57302 aéroports recensés par

 \url{https://ourairports.com/}. Chaque ligne correspond à un aéroport avec les informations 
 \textit{nom;latitude;longitude;altitude (en pieds); code ISO du pays} séparées par un point virgule. Voici les trois premières lignes :
...
\item \texttt{photos.txt} contient la sortie standard d'une commande du \texttt{shell} (en l'occurrence \texttt{ls -l}) listant le contenu d'un répertoire contenant 123 photos. Chaque ligne regroupe diverses informations concernant un fichier contenues dans des champs séparés par des espaces. Le nom du fichier figure dans le dernier champ et sa taille en octets dans le cinquième. Voici les trois premières lignes :

\begin{lstlisting}[style=compil, numbers=left]
-rw-r--r-- 1 frederic frederic 2422388 mai   14 15:06 DSC00001.JPG
-rw-r--r-- 1 frederic frederic 2532569 juil. 13 14:00 DSC00002.JPG
-rw-r--r-- 1 frederic frederic 3159257 juil. 13 15:10 DSC00003.JPG
\end{lstlisting}

\end{itemize}


L'objectif des exercices suivants est d'extraire les informations de chacun de ces fichiers sous la forme de \textbf{tableaux de tuples} :
...
(123, [('-rw-r--r--','1','frederic','frederic','2422388','mai','14','15:06','DSC00001.JPG'),
  ('-rw-r--r--','1',frederic','frederic','2532569','juil.','13','14:00','DSC00002.JPG')])
\end{lstlisting}


{\itshape \bcattention{} On peut remarquer qu'il faut connaître la signification des différents champs constituant une ligne pour l'exploiter. Cette information est en général contenue dans une ligne d'en-tête qui a été enlevée de chaque fichier. En effet, on ne dispose pas encore d'un type de séquence à champs nommés, qui permettrait d'accéder à ses éléments par des descripteurs de type texte plutôt que par des index numériques. En Python, ces tuples à champs nommés sont représentés par les dictionnaires que nous introduirons dans le prochain chapitre. }


\begin{exercice}{}

\begin{enumerate}
	\item Dans un IDE Python, ouvrir le   script \texttt{TP-puplets-eleves.py} fourni  dans l'archive \texttt{materiel.zip} Tous les codes du TP seront écrits dans ce script, en séparant si possible les différents exercices en cellules.
	\item Saisir puis exécuter les instructions suivantes dans l'éditeur, afin d'afficher dans la console le contenu du fichier \texttt{communes69.csv}. On fournit en Annexe un memento de manipulation des fichiers textes.
...
\item Saisir puis exécuter les  instructions suivantes dans la console :
...
\begin{itemize}
	\item lecture de la première ligne uniquement avec \texttt{ligne1 = f.readline()};
	\item suppression du caractère de fin de ligne avec \texttt{ligne1 = ligne1.rstrip()};
	\item découpage de la ligne en une liste de champs selon le séparateur \texttt{','} et conversion en \texttt{tuple} avec \texttt{tuple\_ligne = tuple(ligne1.split(','))}
\end{itemize}

\item Écrire la  fonction dont on donne le prototype ci-dessous :
...
Deux   assertions sont fournies dans \texttt{TP-puplets-eleves.py} pour tester la fonction.
...
\begin{enumerate}
	\item Exécuter le code suivant dans la console. Quel problème peut-on remarquer ?
\begin{lstlisting}[style=compil]
...
\item Une solution est d'utiliser une expression régulière  (ou regex) comme séparateur et de remplacer la fonction \texttt{str.split} par \texttt{re.split} du module \texttt{re}.  Une expression régulière, est une formule décrivant un ensemble de chaînes de caractères.

Écrire une fonction de signature \texttt{fichier\_vers\_tab\_tuple2(fichier, separateur, regex)} avec un paramètre booléen \texttt{regex} qui permet de découper selon une expression régulière.
...
\section{Recherches dans un tableau de tuples}

\begin{exercice}{}

%Avec \texttt{fichier\_vers\_tab\_tuple(\texttt{communes69.csv}, ',')} on peut extraire un tableau de tuples rassemblant tous les enregistrements contenus dans le fichier \texttt{communes69.csv}.

Pour  traiter les informations d'un tableau de tuples, on peut utiliser le \textit{tuple unpacking} mais attention \bcattention{} tous les champs de tuple sont  de type chaînes de caractères \texttt{str} et il faut d'abord les convertir dans le type correspondant au traitement souhaité.
...
'3403'                 
>>> int(pop_int) + int(pop_ext)  #calcul correct de la population totale après conversion en entiers
343
\end{lstlisting}



Les fonctions doivent être testées sur les assertions fournies dans \texttt{TP-puplets-eleves.py}.

Pour chaque fonction le paramètre \texttt{tab} est un tableau de tuples rassemblant tous les enregistrements contenus dans le fichier \texttt{communes69.csv}.
\begin{enumerate}



	\item Écrire une fonction \verb+population_minimale(tab)+  qui retourne un tuple constitué de la population minimale et un tableau des  noms des  communes atteignant ce minimum. 
	\item Écrire une fonction \verb+population_cumul(tab)+  qui retourne un tuple constitué des cumuls des populations municipale, externe    et totale des communes.	
	\item Écrire une fonction \verb+population_moyenne(tab)+ qui retourne un tuple constitué des moyennes des populations municipale, externe     et totale des communes.
	
	\item Écrire une fonction \verb+population_maximale(tab)+  qui retourne un tuple constitué de la population maximale et un tableau des  noms des  communes atteignant ce maximum. Tester puis critiquer le résultat.
	
	\item Écrire une fonction \verb+population_lyon(tab)+ qui retourne la population totale de la ville de Lyon regroupant neuf communes d'arrondissement. 
...
Avec \texttt{fichier\_vers\_tab\_tuple(\texttt{airports-reduit.csv}, ',')} on peut extraire un tableau de tuples rassemblant tous les enregistrements contenus dans le fichier \texttt{airports-reduit.csv}.

Les fonctions doivent être testées sur les assertions fournies dans \texttt{TP-puplets-eleves.py}.

\bcattention{} Dans certains cas, certains valeurs de tuple sont manquantes ! Il faut prévoir ces cas dans le traitement.

\begin{enumerate}

\item Écrire une fonction \verb+nombre_aeroports_pays(tab, pays)+ dont on donne ci-dessous le prototype :
...
\item Un pied correspond à 1/3 de verge anglaise (yard), c'est-à-dire $0,3048$ mètre. Écrire une fonction qui convertit une mesure en mètres en une mesure en pieds.

\item Écrire une fonction \verb+filtre_altmin_aeroports(tab, altmin)+  vérifiant le prototype ci-dessous :
...
\item Écrire une fonction \verb+filtre_altmax_aeroport+ vérifiant le prototype ci-dessous :
...
def filtre_altmax_aeroport(tab, latmin):
    ...
    où altmax est l'altitude maximale d'un aeroport de latitude > latmin
...
\item Peut-on facilement déterminer le pays qui compte le plus d'aéroports dans  \texttt{airports-reduit.csv} ?
...
\begin{methode}{Lecture / écriture de fichiers textes}

En Python, l'accès à un fichier texte se fait par l'intermédiaire d'un descripteur de fichier créé à l'aide de la primitive \texttt{open(nom,mode)}. Une fois que les manipulations sont terminées, il faut bien penser à fermer le descripteur de  fichier avec  \texttt{f.close()}. 
...
Pour bien manipuler un fichier texte, il faut d'abord conna\^itre la façon dont il est structuré !!!
\vspace*{-20pt}
{\small
\begin{center}
\begin{longtable}{|l|l|}
...
\hline
\texttt{f.readlines()} & lecture de toutes  les lignes stockées dans une liste  \\
\hline
