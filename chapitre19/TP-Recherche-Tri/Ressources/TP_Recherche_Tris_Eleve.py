...
def recherche_attributs_condition(table, attribut, valeur, condition):
    for enregistrement in table:
        if condition(enregistrement, attribut, valeur):
            return True
    return False

def condition1(enregistrement, attribut, valeur):
    return enregistrement[attribut[0]]  == valeur[0] and enregistrement[attribut[1]] == valeur[1]


def condition2(enregistrement, attribut, valeur):
    return enregistrement[attribut[0]]  == valeur[0] or enregistrement[attribut[1]] == valeur[1]


#postconditions
def test_recherche_attributs_condition(table):
    assert recherche_attributs_condition(table_clients, ["prénom", "département"], ["Frédéric", "69"], condition1) == True
    assert recherche_attributs_condition(table_clients, ["prénom", "département"], ["Nestor", "96"], condition2) == False
