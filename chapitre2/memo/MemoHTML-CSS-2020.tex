...
Une  page Web est un document de structure non linéaire (lien hypertexte), avec des contenus riches (images, video) et des interactions possibles côté client (navigateur) ou côté serveur (accès à une base de données).

On distingue les pages Web \textbf{statiques} dont le contenu ne dépend de l'utilisateur et de ses interaction, des pages Web \textbf{dynamiques}. 

Un \textbf{principe de séparation} des fonctionnalités est appliqué dans  la  réalisation d'une page Web :

\begin{itemize}
	
	\item La structure du document est défini par le langage \texttt{HTML}. Pour les pages Web dynamiques, le code \texttt{HTML} peut être généré par le serveur et envoyé au client à sa demande.
	\item L'apparence et la disposition des éléments sont définis par le langage \texttt{HTML}.
...
Certaines balises ont une \textbf{sémantique} prédéfinie : \texttt{<article>}, \texttt{<section>}, très utile pour l'indexation de la page par un moteur de recherche. D'autre balises comme \texttt{<span>} (en-ligne)  ou \texttt{<div>} (bloc) sont neutres et personnalisables par le développeur.
\end{cours}
...
\hline 
<strong> & texte important & <strong>important</strong> \\ 
\hline 
...
\begin{itemize}
	...
	\item Un \textbf{identifiant }avec le préfixe \verb+#+  pour cibler l'unique élément   qui a été marqué par cet identifiant dans le document \texttt{HTML} avec la syntaxe \texttt{<elment id="monidentifiant">}.
		\item Une \textbf{classe} avec le préfixe .  pour cibler un ensemble d'éléments, pas forcément de même type,   qui ont  été marqués par cette classe dans le document \texttt{HTML} avec la syntaxe \texttt{<elment class="uneclasse">}.
